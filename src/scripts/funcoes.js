function selecionarLata(valorMlSelecionado) {

    limparBotoes();

    mlSelecionado = valorMlSelecionado

    document.getElementById(`botao-${valorMlSelecionado}`).classList.add('botao-ativo');
  ...
  function limparBotoes(){
    for (let lata of listaLata) {
        document.getElementById(`botao-${lata.ml}`).classList.remove('botao-ativo');  
    }
  }
  ...
  function adicionarBotao(lata) {
    const botaoHtml = `
                      <div id="botao-${lata.ml}" onclick="selecionarLata(${lata.ml})" class="bolinha" value="${lata.ml}">${lata.tipo} ${lata.ml} ml</div>
                  `;
